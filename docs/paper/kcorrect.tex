...
ultraviolet, optical, and near-infrared observations in the redshift
range $0 < z < 1.5$. Since we base the templates on stellar population
...
estimate $K$-corrections, stellar masses, and recent star-formation
rates. This software is suitable for analyzing many other data sets as
well.
\end{abstract}
...
When different entries $\Xij$ of the data matrix have different
...
\beq
\chi^2 = \sum_{ij} \left( \frac{\Xij - 
\sum_k \Wik\Hkj}{\sij}\right)^2
\eeq
...
%In particular, define $\XXt_{ij}=\Xij/\sij$,
%$\WWt(j)_{ik}=\Wik/\sij$ and 
%$\HHt(i)_{kj}=\Hkj/\sij$. 
In particular, when updating $\WW$, we can rewrite the cost function as
\beq
\chi^2 = \sum_{ij} \left( \frac{\Xij}{\sij} - 
\sum_k \Wik\frac{\Hkj}{\sij}\right)^2
%= \sum_{ij} \left(\XXt_{ij} - 
%\sum_k \Wik\HHt(i)_{kj}\right)^2
\eeq
yielding updates of the form:
\beq
\Wik \leftarrow \Wik 
\left( \sum_j \frac{\Xij\Hkj}{\sij^2} \right) / 
\left( \sum_{mj} \frac{W_{im}H_{mj}\Hkj}{\sij^2}\right)
%\Wik \leftarrow \Wik \frac{[\XXt\HHt(i)\T]_{ik}}{[\WW\HHt(i)\HHt(i)\T]_{ik}}
\eeq

and similarly, when updating $\HH$, we can rewrite the cost function as
\beq
\chi^2 = \sum_{ij} \left( \frac{\Xij}{\sij} - 
\sum_k \frac{\Wik}{\sij}\Hkj\right)^2
%= \sum_{ij} \left(\XXt_{ij} - 
%\sum_k \WWt(j)_{ik}\Hkj\right)^2
\eeq
yielding updates of the form:
\beq
\Hkj \leftarrow \Hkj
\left( \sum_i \frac{\Wik\Xij}{\sij^2}\right) /
\left( \sum_{mi} \frac{\Wik{}W_{im}H_{mj}}{\sij^2}\right)
%\Hkj \leftarrow \Hkj \frac{[\WWt(j)\T\XX]_{kj}}{[\WWt(j)\T\WWt(j)\HH]_{kj}}
\eeq

This argument can be equally applied to our extended model, yeilding
the final update equations which are actually implemented in the
kcorrect code:
\centerline{\fbox{\parbox{4.3in}{
\beqa 
\Aik &\leftarrow \Aik 
\left( \sum_{rj} \frac{\Xij\Mrj\Bkr}{\sij^2}\right) / 
\left( \sum_{mnrj} \frac{A_{im}B_{mr}M_{rj}M_{jn}B_{kn}}{\sij^2}\right) \\
\Bkr &\leftarrow \Bkr
\left( \sum_{ij} \frac{\Aik\Xij\Mrj}{\sij^2}\right) / 
\left( \sum_{mnij} \frac{A_{ik}A_{im}B_{mn}M_{nj}M_{rj}}{\sij^2}\right)
\eeqa
}}}
...
%\eeqa
%}~~}}\\[.2in]
%pwhere $\XXt_{ij}=\Xij/\sij$, $\MMt(i)_{rj} = \Mrj/\sij$ and
%$\AAt(?)_{ik}=\Aik/??$. 
%}}}
