...
;       binary isochrones.  Using the ISEDFILE and ISEDPATH optional
;       inputs you can read in an arbitrary BC03 SED in binary
;       format. 
;
; BUGS:
...
; and also to determine on which row the data begin 
    ...
    if (nheadlines eq 0L) then message, 'There is a problem with '+extrapath+extrafile+'.'

    agerow = where(strmatch(temp,'*log-age*',/fold) eq 1B,nagerow)
    if (nagerow ne 1L) then message, 'There is a problem with '+extrapath+extrafile+'.'
...
; read the data
    ...
    if (ncheck ne ncols) then message, 'There is a problem reading '+extrapath+extrafile+'.'
...
function k_im_read_bc03, isedfile=isedfile, isedpath=isedpath, metallicity=metallicity, $
  age=age, minwave=minwave, maxwave=maxwave, salpeter=salpeter, lr=lr, silent=silent, $
...
;      print, i1, i2, (i2-i1)+1
    ...
    if (ninputage ne 0L) then begin
       ...
       if not keyword_set(silent) then begin
          ...
          for j = 0L, ninputage-1L do print, '   '+string(bc03.age[indx[j]]/1E9,format='(F12.5)')+' Gyr'
       endif
