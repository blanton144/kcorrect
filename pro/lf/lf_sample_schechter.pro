...
;-
pro lf_sample_schechter,ndesired,schechter,p,q,sample_absmmin,sample_absmmax, $
                        sample_zmin,sample_zmax, $
                        absmout=absmout,zzout=zzout,fpadjust=fpadjust, $
                        omega0=omega0,omegal0=omegal0,zzero=zzero, $
                        nzvals=nzvals,rhozpars=rhozpars,rhozfunc=rhozfunc, $
                        zvals=zvals,sample_nzvals=sample_nzvals
...
; find maximum value of schechter
absmk=sample_absmmin+(sample_absmmax-sample_absmmin)* $
  (lindgen(1000)+0.5)/double(1000)
phimax=max(lf_schechter(absmk,schechter.phistar,schechter.mstar, $
                        schechter.alpha))
...
; choose an absolute magnitude for each object
    npick=nleft
    tmp_absm=sample_absmmin+(sample_absmmax-sample_absmmin)* $
      randomu(seed,npick)
    indx=where(randomu(seed,npick) lt $
               lf_schechter(tmp_absm,schechter.phistar,schechter.mstar, $
                            schechter.alpha)/phimax,count)
    
    if(count gt 0) then begin
        npick=count
        tmp_absm=tmp_absm[indx]
