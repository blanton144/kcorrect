...
;-
;------------------------------------------------------------------------------
pro k_nmf_spdata_photoz, mmatrix=mmatrix, sample=sample, flux=flux, $
...
if(n_elements(sample) eq 0) then sample='drtwo14'
if(n_elements(flux) eq 0) then flux='model'
if(keyword_set(few)) then begin
    if(n_elements(nlrg_photo) eq 0) then nlrg_photo=500L
    if(n_elements(nlrg_spec) eq 0) then nlrg_spec=0L
    if(n_elements(nsdss_photo) eq 0) then nsdss_photo=2200L
    if(n_elements(nsdss_spec) eq 0) then nsdss_spec=0L
    if(n_elements(ngalex) eq 0) then ngalex=500L
    if(n_elements(ndeep) eq 0) then ndeep=500L
    if(n_elements(ngoods) eq 0) then ngoods=500L
...
;; sdss photometry
    ...
    postcat=postcat[ii]
    indx_photo=shuffle_indx(n_elements(postcat), num_sub=nsdss_photo, $
                            seed=seed)
    ...
    sp=spfull[postcat.object_position]
    vmod=mrdfits(getenv('VAGC_REDUX')+'/velmod_distance/distance_'+ $
                 velmodtype+ '.fits',1, row=postcat.object_position)
