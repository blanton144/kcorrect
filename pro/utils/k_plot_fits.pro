...
tspec_t=mmatrix#templates
absrc=3.631*2.99792*1.e-2/lambda^2
for i=0L, nt-1L do $
  tspec_t[*,i]=tspec_t[*,i]*absrc

np=1

;; read in some galaxies 
lowz=lowz_read(sample='drtwo14')
ii=where(lowz.absmag[2] lt -17. and lowz.absmag[2] gt -18. and $
         abs(lowz.absmag[1]-lowz.absmag[2]-0.4) lt 0.05, nii)
ipick=ii[shuffle_indx(nii,num_sub=np)]
im=mrdfits(vagc_name('object_sdss_imaging'), 1, $
           row=lowz[ipick].object_position)
kc=sdss_kcorrect(lowz[ipick].zdist, calibobj=im, coeffs=coeffs)

tspec=tspec_t#coeffs

colors=['blue', 'green', 'magenta', 'red']

;; for each template, show the spectrum (full and just optical)
k_print, filename='fit_ex_spec.ps', axis_char_scale=1.4
!P.MULTI=[0, 1, 2]
!Y.MARGIN=[1,0]
...
sfh_tot_t=fltarr(nages,nt)
for i=0L, nt-1L do $
  for j=0L, nages-1L do sfh_tot_t[j,i]=total(sfh[j,*,*,i])
sfh_met_t=fltarr(nages,nt)
for i=0L, nt-1L do $
  for j=0L, nages-1L do sfh_met_t[j,i]=total(sfh[j,*,*,i]* $
                                             metallicities[met[j,*,*]])
sfh_met_t=sfh_met_t/sfh_tot_t
sfh_dust_t=fltarr(nages,nt)
for i=0L, nt-1L do $
  for j=0L, nages-1L do sfh_dust_t[j,i]=total(sfh[j,*,*,i]* $
                                            dust[j,*,*].tauv)
sfh_dust_t=sfh_dust_t/sfh_tot_t

sfh_tot=sfh_tot_t#coeffs
sfh_met=((sfh_met_t*sfh_tot_t)#coeffs)/sfh_tot
sfh_dust=((sfh_dust_t*sfh_tot_t)#coeffs)/sfh_tot
